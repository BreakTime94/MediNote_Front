...
export default function useCheckPassword(oldPassword, delay = 500) {

  // 사용자의 입력값을 일정 시간 동안 묵혀두는 디바운스용 값
  const [debouncedValue, setDebouncedValue] = useState(oldPassword);
  ...
  useEffect(() => {
    const handler = setTimeout(() => setDebouncedValue(oldPassword), delay);
  ...
  useEffect(() => {
    ...
    axios.post("api/member/check/password", { oldPassword: debouncedValue })
        .then((resp) => {
