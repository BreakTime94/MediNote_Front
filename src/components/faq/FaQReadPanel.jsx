...
export default function FaQReadPanel({
                                         onBack,
                                         showAdminActions = true,          // 권한 제어는 이후에
                                         listPath = "/boards/faq",         // 라우터에 맞춰 기본 경로
                                     }) {
    const { id } = useParams();
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            // FAQ 카테고리 기본 3 (서버값이 있으면 그 값을 우선)
            const dto = {
                id: Number(id),
                boardCategoryId: data?.boardCategoryId ?? 3,
                title: title.trim(),
                content: content.trim(),
                isPublic: data?.isPublic ?? true,                // null 패치 방지
                requireAdminPost: data?.requireAdminPost ?? false,
                qnaStatus: data?.qnaStatus ?? "WAITING",         // DTO 필드 존재로 기본값 유지
