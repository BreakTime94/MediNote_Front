...
function MeasurementEdit({ mode = "page", id: propId, presetData, onBack }) {
  const navigate = useNavigate();
  const params = useParams(); // 메인 라우팅용
  const id = mode === "mypage" ? propId : params.id; // ✅ (1) id 출처 분기
  ...
  useEffect(() => {
    if (presetData) {
      setForm(presetData); // ✅ 이미 데이터 있으면 fetch 안 함
      setLoading(false);
      return;
    }
  ...
  const handleSaveNewVersion = async () => {
    ...
    try {
      ...
      if (mode === "mypage") onBack?.();
      else navigate("/health/measurement/list");
    } catch (err) {
